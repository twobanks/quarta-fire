...
export default function AccordionInscricao({ action }: AccordionInscricaoProps) {
  ...
  const [erro, setErro] = useState<string | null>(null) // Novo estado para erros
  ...
  function handleSubmit(formData: FormData) {
    ...
    startTransition(async () => {
      ...
      // Se a action retornar um erro, exibe na tela e para por aqui
      if (response?.error) {
        setErro(response.error)
        return
      }

      // Se for sucesso, dispara o grito de guerra!
      if (response?.success) {
        setSucesso(true)
        setTimeout(() => {
          setSucesso(false)
